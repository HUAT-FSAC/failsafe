...
{
    // ros::Subscriber cam_sub = nh.subscribe("/pylon_camera_node/image_raw", 1, cam_callback);
    ros::Subscriber lidar_sub = nh.subscribe("/velodyne_points", 2, lidar_callback);
    ros::Subscriber imu_sub = nh.subscribe("/INS/ASENSING", 1, imu_callback);

    ros::Publisher control_pub = nh.advertise<common_msgs::vehicle_cmd>("failsafe_control", 100);
}
